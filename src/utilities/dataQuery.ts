...
class DataQuery {
    ...
    public async getAggregatedItems<Type>(query:any = {}, project:any = {}, options:any = {}):Promise<IListOutput<Type>> { // eslint-disable-line @typescript-eslint/no-explicit-any
        const output:IListOutput<Type> = {
            items: [],
            totalItems: 0,
            page: 0,
    ...
    // todo: aggregation query
    public async getAggregatedItemsByPage<Type>(query:any, project:any, options:any, pageOptions:IPgeInfo):Promise<IListOutput<Type>> { // eslint-disable-line @typescript-eslint/no-explicit-any
        options.skip = (pageOptions.page - 1) * pageOptions.pageSize
        options.limit = pageOptions.pageSize

        const output = await this.getAggregatedItems<Type>(query, project, options)
